...
import {reserve} from './lib/reserve-env';
...
const getDeploymentEnv = async (argv, commit)=> {
  const [deployEnv, deployPath] = argv['deploy-env'].split('/');

  if (deployEnv === 'auto') {
    const token = argv['github-token'];
    const envKey = await reserve(commit, token);
    return {deployEnv: `apps.skandev.io`, deployPath: envKey};
  }

  return {deployEnv, deployPath: deployPath || 'recipes'};
};
...
const main = async (process)=> {
  const {argv} = yargs(process.argv.slice(2))
    ...
    .option('deploy-env', {
      ...
      default: 'apps.skandev.io/recipes'
    })
    .options('node-env', {
  ...
  const {deployEnv, deployPath} = await getDeploymentEnv(argv, buildCommit);
